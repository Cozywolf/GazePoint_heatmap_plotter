# Following functions are modified from https://github.com/TobiasRoeddiger/GazePointHeatMap by TobiasRoeddiger to work with Python 3
...
def draw_display(dispsize, imagefile=None):
    ...
    if imagefile != None:
        ...
        # print(type(x), type(y), type(h), type(w))
...
def draw_heatmap(gazepoints, dispsize, imagefile=None, alpha=0.5, savefilename=None, gaussianwh=200, gaussiansd=None):
    ...
    for i in range(0, len(gazepoints)):
        ...
        if (not 0 < x < dispsize[0]) or (not 0 < y < dispsize[1]):
            ...
            try:
                heatmap[int(y):int(y) + vadj[1], int(x):int(x) + hadj[1]] += gaus[vadj[0]:vadj[1], hadj[0]:hadj[1]] * gazepoints[i][2]
            except:
